...
void defects::VertexProximityDetection::detect(const MeshData& mesh)
{
	...
	const double inv_threshold = 1.0 / threshold;
	const double sqrd_threshold = threshold * threshold;
	...
	for (Eigen::Index i = 0; i < vertices.rows(); ++i)
	{
		...
		// quantized voxel
		Eigen::Vector3i voxel = (vi * inv_threshold).array().floor().cast<int>();
		...
		for (int dx = -1; dx <= 1; ++dx) for (int dy = -1; dy <= 1; ++dy) for (int dz = -1; dz <= 1; ++dz)
		{
			...
			if (it != voxel_grid.end())
			{
				for (Eigen::Index j : it->second)
				{
					...
					if (sqrd_distance <= sqrd_threshold)
